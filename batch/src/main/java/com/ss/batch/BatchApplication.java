...
@EnableBatchProcessing// spring batch 기능을 활성화 한다.
	...
	public Step passStep() {
		return this.stepBuilderFactory.get("passStep").tasklet(new Tasklet() {
			...
			public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
				System.out.println("Execute PassStep!실행");
 ...
 * - 대량의 데이터를 처리하는 작업을 의미한다. 자동화하여 시스템의 부하를 줄이고 효율적인 데이터를 처리
 * 
 * 배치 프로그램
 * - 대량의 데이터를 처리하는 작업을 자동화하는 프로그램
 * 
 * 스케줄러(Scheduler)
 * - 일정한 시간 간격으로 반복적으로 수행되거나 특정 시간에 예약해 놓은 작업을 자동으로 실행해주는 시스템 
 * 
 * 스프링 배치는 처리하는 세 가지 단계
 * - 1. 읽기 : 파일, 데이터 베이스 등에서 데이터를 읽는다.
 *   2. 처리 : 읽은 데이터를 필요한 형태로 변환하거나 처리한다.
 *   3. 쓰기 : 처리 된 데이터를 파일이나 데이터베이스로 쓴다.
 *   데이터가 없을 때 까지 무한적으로 반복함
 *   
 *  위에서 데이터를 처리하다가 에러 발생하면 어던 작업을 하다가 에러가 났는지 아님 정상적으로 수행했는지
 *  작업을 기록하는 테이블 메타 데이터
 *  
 *  청크(Chunk)
 *  -데이터를 여러개의 묶음으로 처리
 *  -사이즈를 내가 설정을 할 수 있다. chunk(사이즈) ex: chunk(10)
 *  -대량의 데이터를 처리할 때 사용한다.
 *  -메모리를 효율적으로 사용한다.*
 *  
 *  태스크릿(Tasklet)
 *  -단일작업 처리하는데 사용함
 *  -파일삭제, 데이터를 정리하는 단일 작업
 ...
 *     # 데이터베이스 스키마를 자동으로 초기화 할 때 사용하는 설정
 *       필요한 테이블을 자동으로 생성해주고 초기화 진행!
 *       spring.batch.jdbc.initialize-schema=always
 *       
 *    이용권을 기준으로 해서 데이터 설계
 *    여러개의 헬스장에서 이 프로그램을 사용할 수 있도록!
 *    N개의 체육관을 기준으로 해서 테이블 생성
 *    
 *    체육관
 *    이름, 장소,상태
 *     
 ...
 *    
 *    예약
 *     예약순번, 체육관ID,사용자ID,이용권순번, 시작일시,종료일시 이용권 차감 여부! 
 *    
 *     
 *   
 *  
