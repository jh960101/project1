...
// MapStruct가 매핑 인터페이스를 인식하도록 설정한다.
...
public interface NotificationModelMapper {
	...
	// MapStruct가 자동으로 생성한
	// NotificationModelMapper 객체를 사용할 수 있도록
	// INSTACE 이용해서 매퍼 사용할 수 있음
	NotificationModelMapper INSTACE = Mappers.getMapper(NotificationModelMapper.class);

	// 필드명이 같지 않거나 커스텀하게 매핑해주기 위해서는 어노테이션 @Mapping을 추가해주면 된다.
	...
	// 매핑을 하는 이유는 예약정보(Booking)를 기반으로 알림을 생성하고 발송하기 위해서 이과정에서 예약과 관련된
	// 중요한 정보가 있다. 중요정보를 (NotificationEntity) 알림을 전달한다.
}
