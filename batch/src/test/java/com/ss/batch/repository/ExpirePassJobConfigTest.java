...
public class ExpirePassJobConfigTest {
	// 배치 작업을 테스트 할 때 Job을 실행하고 그 결과를 가져오는데 사용된다.
	@Autowired
	...
	public void test_expirePassStep() throws Exception {
		...
		// then 검증
		// assertEquals(값,값)두개의 값이 같은지 확인하는 메서드.junit이 제공
		assertEquals(ExitStatus.COMPLETED, jobExecution.getExitStatus());
		// 내가 직접 설정한 job이 실행했는지 확인
		assertEquals("expriedPassJob", jobInstance.getJobName());
	...
	private void addPassEntities(int size) {
		...
		for (int i = 0; i < size; ++i) {
			...
			passEntity.setStatus(PassStatus.PROGESES);
