...
public class AddPassTaskletTest {
	...
	@Test
	public void test_excute() throws Exception {
		// given
		...
		// when
		// any()
		// - 특정 타입의 인자가 무엇이든 상관없이 메서드를 실행 할 수있다ㅏ. 모든 인자값을 허용
		// 지정한 타입이 맞는지 맞으면 동적하는 설정.
		when(bulkPassRepository.findByStatusAndStartedAtGreaterThan(eq(BulkPassStatus.READY), any()))
				.thenReturn(List.of(bulkPassEntity));
		// 그룹에 속한 그룹ID에 사용자 찾기
		// eq(userGroupId) 만약 일치가 되지않으면 검색안함
		// 만약 문자가 일치하면 데이터를 검색하도록 설정

		// when()
		// 검색을 했을 때 데이터가 있으면 그 데이터를 thenReturn()메서드로 전달한다.
		when(groupRepo.findByUserGroupId(eq(userGroupId))).thenReturn(List.of(userGroupEntity));

		// 매핑 PassModelMapper이용해서 벌크=> 패스 엔티티로 변환
		// 배치 작업시 현재 작업에 대한 상태와 정보를 전달하는 역할로 쓰인다.
		...
		// PassEntity객체들이 들어있다.
		// verify 특ㄹ정 메서드 호출되었는지 여부를 확인할 때 사용
